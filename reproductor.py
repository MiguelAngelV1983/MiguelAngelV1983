...
class Player:
    ...
    def __init__(self):
        ...
        Button(self.root,text="BUSCAR",width=78,bg="blue",fg="white",command=self.open_file).place(x=356,y=75)
        ...
        Button(self.root,text="AÑADIR A LA PLAYLIST",width=44,bg="goldenrod1",command=self.add).place(x=594,y=108)

        self.items = Label(self.root,text=('{} CANCIONES'.format(len(self.audio_list))),font=("arial",10),width=39,height=2,bg="black",fg="red")

        self.items.place(x=594,y=147)

        Button(self.root,text="ELIMINAR PLAYLIST",width=44,command=self.remove_playlist).place(x=594,y=220)

        Button(self.root,text="ELIMINAR DESDE PLAYLIST",width=44,command=self.remove_from_list).place(x=594,y=190)

        self.btnPlayall = Button(self.root,text="REPRODUCIR TODO",width=21,height=2,command=self.init_task2)

        self.btnPlayall.place(x=594,y=254)

        self.btnRandom = Button(self.root,text="MODO ALEATORIO: OFF",width=21,height=2,command=self.act_random)
        ...
        if self.playall_mode == False and self.playing == False:
            ...
            if self.file_path: #and self.playing == False:
                ...
                if os.path.exists(self.file_path):
                    ...
                    messagebox.showwarning("ARCHIVO NO ENCONTRADO",'''Ruta no encontrada, el archivo ha sido movido o borrado.''')
    ...
    def remove_playlist(self):
        ...
        if self.fav_list.size() > 0:
            ...
            message = messagebox.askquestion("ELIMINAR PLAYLIST",'Está seguro de querer borrar toda la lista?')
            ...
            if message == "yes":
                ...
                self.btnPlayall.configure(text="REPRODUCIR TODO")
    ...
    def remove_from_list(self):
        ...
        if self.fav_list.size() > 0:
            ...
            if self.any_selected:
                ...
                message = messagebox.askquestion("ELIMINAR ELEMENTO",'¿Desea borrar el elemento seleccionado de la lista?')
                ...
                messagebox.showwarning("NO HAY ELEMENTOS ELECCIONADOS","Seleccione el elemento de la lista que desea borrar.")
    ...
    def open_file(self):
        ...
        fpath = filedialog.askopenfilename(initialdir = "/",title = "Seleccione archivo",

                        filetypes = (("ficheros mp3","*.mp3"),("Todos los archivos","*.*")))
        ...
        if fpath:
            ...
            if fpath.endswith(".mp3"):
                ...
                messagebox.showwarning("ERROR","Formato de fichero incorrecto.")
    ...
    def init_task2(self):
        ...
        if self.playall_mode == False and self.playing == False:
            ...
            if self.fav_list.size() > 0:
                ...
                self.btnPlayall.configure(text="REPRODUCIENDO TODO...")
                ...
                messagebox.showwarning("LISTA VACÍA","No hay elementos en la Playlist.")
    ...
    def act_random(self):
        ...
        if self.random_mode == True:
            ...
            self.btnRandom.configure(text="MODO ALEATORIO: OFF")

        else:

            self.random_mode = True

            self.btnRandom.configure(text="MODO ALEATORIO: ON")
    ...
    def music(self):
        ...
        try:
            ...
            messagebox.showwarning("ERROR INESPERADO",str(e))
